    ...
    try {
        if (ctx.session.step === 'select_ad_type') {
            ...
            if (!isValidMarkdown(message)) {
                ctx.reply('⚠️ نشانه‌گذاری Markdown ناقص است. لطفاً مطمئن شوید که تمام * و _ به‌درستی بسته شده‌اند (مثلاً *متن* یا _متن_). دوباره امتحان کنید:', { reply_markup: { remove_keyboard: true } });
                return;
            }
...
const usernameHandler = async (ctx) => {
    ...
    try {
        ...
        await container_1.registerProject.execute(telegramId, title, description, budget, deadline || '', 'gateway', ctx.telegram, message, role, // role قبل از پارامترهای اختیاری
        adType, adType === 'paid' ? amount : undefined, isPinned || false);
        if (adType === 'free') {
