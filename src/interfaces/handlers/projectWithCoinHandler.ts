...
export const projectWithCoinHandler = async (ctx: CustomContext) => {
...
export const usernameHandler = async (ctx: CustomContext) => {
    ...
    try {
        await registerProject.execute(telegramId, description, 'رایگان', deadline, 'gateway', ctx.bot, message);
        ctx.reply(
